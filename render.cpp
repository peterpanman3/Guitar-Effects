...
#define SW_CTXT_ENVF			9
#define SW_CTXT_FBCOMP			10
#define SW_CTXT_LOOPER			11
#define SW_CTXT_VOLUME			12
...
#define EF_ADSR_STN			3
	//Looper page 1 (only volume controls implemented)
#define LOOP_VOL			0
...
//Control context 
#define CONTROL_EFFECT_SWITCH  	0
#define EFFECT_BYPASS_SWITCH	1
#define CONTROL_CONTEXT_SWITCH	2
#define LOOP_REC_SWITCH			3
#define LOOP_CLR_SWITCH			4
//#define WAH_PEDAL_TOGGLE		3
...
void set_efx_looper()
{
	...
	if(knobs[LOOP_VOL].active_timer != 0) {
		volume = map(knobs[LOOP_VOL].filtered_reading[1], 0.0, 1.0, 0.0, 1.0);
		looper.looper_set_volume(volume);
		doprint = true;
	}
	
	if( (rtprintclock >= 20000) && (doprint) ){
		rt_printf("Looper volume:  %f", volume);
		rtprintclock = 0;
	}
}
...
void apply_settings()
{
	...
	if (pushbuttons[LOOP_REC_SWITCH].rising_edge)
		...
		looper.loopRec = 0;
	pushbuttons[LOOP_REC_SWITCH].rising_edge = false;
		
	if (pushbuttons[LOOP_CLR_SWITCH].rising_edge)
		looper.loopClr = 1;
	else
		looper.loopClr = 0;
	pushbuttons[LOOP_CLR_SWITCH].rising_edge = false;
...
void process_digital_inputs()
{
	...
	if(pushbuttons[CONTROL_EFFECT_SWITCH].rising_edge == true)
	{
		...
		else if (ain_control_effect == SW_CTXT_LOOPER)
			rt_printf("Active Effect: LOOPER, %d\n", ain_control_effect);
		else if (ain_control_effect == SW_CTXT_VOLUME)
			rt_printf("Active Effect: VOLUME PEDAL, %d\n", ain_control_effect);
	}
